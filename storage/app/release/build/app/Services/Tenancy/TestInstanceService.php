 ...
 * `parent_tenant_id` pointing at the live tenant — so URL-prefix routing,
...
class TestInstanceService
{
    ...
    /**
     * The URL-prefix a test instance of $live occupies: the live slug with
    ...
    {
        if (is_string($type)) {
            $target = collect(config('tenancy-clone.polymorphic_aliases', []))[$type] ?? null;
            if ($target !== null) {
                return $target;
            }
        }
    ...
    {
        $headline = ['users' => 'users', 'customers' => 'customers', 'jobs' => 'jobs', 'invoices' => 'invoices', 'services' => 'services'];
        $out = [];
        $total = 0;
        $droppedRows = 0;
